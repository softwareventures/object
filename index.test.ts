...
test("mergeRecords", t => {
    const s = Symbol();
    const a: Record<Key, boolean | number | string> = {a: 1, b: 2, [s]: false};
    const b = {a: "hello", c: 3} as const;
    const c = {a: true, d: 4} as const;
    const d = {c: true, d: 5} as const;

    t.deepEqual(mergeRecords([a]), {a: 1, b: 2, [s]: false});
    t.deepEqual(mergeRecords([a, b]), {a: "hello", b: 2, c: 3, [s]: false});
    t.deepEqual(mergeRecords([a, b, c]), {a: true, b: 2, c: 3, d: 4, [s]: false});
    t.deepEqual(mergeRecords([a, b, c, d]), {a: true, b: 2, c: true, d: 5, [s]: false});
});
